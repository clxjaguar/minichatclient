...
// A configuration line from a .conf configuration file (parser_rules.conf)
typedef struct {
	char *context;
	clist *rules;
} parser_config_line;

//// Start of the functions prototypes used only in that .c
...
// end of functions' prototypes used only in that file

// Those are the functions defined in the .h:
...
parser_config *parser_get_config(const char filename[]) {
	...
	atts = ini_get_select(myini, filter_config, NULL);
...
// Those are the functions are only defined privately in the top of this file :
